resource "aws_pipes_pipe" "sfn_state_machine_s3_object_created_detect_entities" {
  depends_on = [
    aws_iam_role_policy.pipes_pipe_s3_object_created_detect_entities
  ]
  name     = aws_sfn_state_machine.comprehend_detect_entities.name
  role_arn = aws_iam_role.pipes_pipe_s3_object_created_detect_entities.arn
  source   = aws_sqs_queue.s3_object_created_detect_entities.arn
  target   = aws_sfn_state_machine.comprehend_detect_entities.arn
  target_parameters {
